...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training or cfg.MODEL.Feature_Generation_Mode:
        ...
        if cfg.MODEL.Feature_Generation_Mode:
            last_f, cut_labels, pair_pred,rel_dists_0 = \
                self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
            return {}, [last_f, cut_labels, pair_pred, rel_dists_0], {}
        else:
        ...
        elif cfg.MODEL.reweight_fineloss:
        ...
        if self.cfg.MODEL.ct_loss:
            add_losses, q_feats, cut_labels = add_losses
            output_losses.update(add_losses)
            return roi_features, proposals, [output_losses, q_feats, rel_labels, cut_labels, relation_logits]
        output_losses.update(add_losses)
