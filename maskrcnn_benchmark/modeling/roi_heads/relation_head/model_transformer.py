...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.context_obj = TransformerEncoder_HTCL(self.obj_layer, self.num_head, self.k_dim, self.v_dim, self.hidden_dim, self.inner_dim, self.dropout_rate)
        self.context_edge = TransformerEncoder_HTCL(self.edge_layer, self.num_head, self.k_dim,  self.v_dim, self.hidden_dim, self.inner_dim, self.dropout_rate)
...
class TransformerEncoder_HTCL(nn.Module):
    def __init__(self, n_layers, n_head, d_k, d_v, d_model, d_inner, dropout=0.1):
        super().__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=n_head, dim_feedforward=d_inner, dropout=dropout, batch_first=True)
    ...
    def forward(self, input_feats, num_objs):
        if max(num_objs) < 80:
            ...
            pad_len = max(num_objs)
            num_objs_ = torch.LongTensor(num_objs).to(device).unsqueeze(1).expand(-1, pad_len)
            non_pad_mask = torch.arange(pad_len, device=device).to(device).view(1, -1).expand(bsz, -1).lt(num_objs_) # (bsz, pad_len)
            obj_pad_mask = torch.arange(pad_len, device=device).to(device).view(1, -1).expand(bsz, -1).ge(num_objs_)
            # -- Forward
            enc_output = self.transformer_encoder(input_feats, src_key_padding_mask=obj_pad_mask)
            ...
            if min(num_objs) < 80:
                ...
                short_list = []
                for i in short_idx:
                    short_list = short_list + id_list[sum(num_objs[:i]):sum(num_objs[:i])+num_objs[i]]
                num_short = [num_objs[i] for i in short_idx]
                input_feats = input_feats[short_list].split(num_short, dim=0)
                input_feats = nn.utils.rnn.pad_sequence(input_feats, batch_first=True)
                # -- Prepare masks
                bsz = len(num_short)
                device = input_feats.device
                pad_len = max(num_short)
                num_objs_ = torch.LongTensor(num_short).to(device).unsqueeze(1).expand(-1, pad_len)
                non_pad_mask = torch.arange(pad_len, device=device).to(device).view(1, -1).expand(bsz, -1).lt(num_objs_) # (bsz, pad_len)
                obj_pad_mask = torch.arange(pad_len, device=device).to(device).view(1, -1).expand(bsz, -1).ge(num_objs_)
                # -- Forward
                enc_output = self.transformer_encoder(input_feats, src_key_padding_mask=obj_pad_mask)
                enc_output = enc_output[non_pad_mask]
                output[short_list] = enc_output

            long_idx = [i for i, x in enumerate(num_objs) if x >= 80]
            for i in long_idx:
                enc_output = self.transformer_encoder(original_input_feats[sum(num_objs[:i]):sum(num_objs[:i])+num_objs[i]].unsqueeze(0))
                output[sum(num_objs[:i]):sum(num_objs[:i])+num_objs[i]] = enc_output.squeeze(0)
            return output