...
class SGMeanRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        ...
        mR100_head = np.array(self.result_dict[mode + '_mean_recall_list'][100])[(np.array(ind_head) - 1)].mean()
        mR100_middle = np.array(self.result_dict[mode + '_mean_recall_list'][100])[(np.array(ind_middle) - 1)].mean()
        mR100_tail = np.array(self.result_dict[mode + '_mean_recall_list'][100])[(np.array(ind_tail) - 1)].mean()
        result_str += 'mR100: '
        result_str += ' mR100_head: %.4f; ' % (float(mR100_head))
        result_str += ' mR100_middle: %.4f; ' % (float(mR100_middle))
        result_str += ' mR100_tail: %.4f; ' % (float(mR100_tail))
        ...
        if self.print_detail:
            ...
            for i in ind_head + ind_middle + ind_tail:
                result_str += '({}:{:.4f}) '.format(str(self.rel_name_list[i - 1]),
                                                    self.result_dict[mode + '_mean_recall_list'][100][i - 1])
            result_str += '\n'
            result_str += '--------------------------------------------------------\n'
...
class SGNGMeanRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self, mode):
        ...
        ng_mR100_head = np.array(self.result_dict[mode + '_ng_mean_recall_list'][100])[(np.array(ind_head) - 1)].mean()
        ng_mR100_middle = np.array(self.result_dict[mode + '_ng_mean_recall_list'][100])[
            (np.array(ind_middle) - 1)].mean()
        ng_mR100_tail = np.array(self.result_dict[mode + '_ng_mean_recall_list'][100])[(np.array(ind_tail) - 1)].mean()
        result_str += 'ng-mR100:'
        result_str += 'ng-mR100_head:%.4f; ' % (float(ng_mR100_head))
        result_str += 'ng-mR100_middle:%.4f; ' % (float(ng_mR100_middle))
        result_str += 'ng-mR100_tail:%.4f; ' % (float(ng_mR100_tail))
        ...
        if self.print_detail:
            ...
            for i in ind_head + ind_middle + ind_tail:
                result_str += '({}:{:.4f}) '.format(str(self.rel_name_list[i - 1]),
                                                    self.result_dict[mode + '_ng_mean_recall_list'][100][i - 1])
